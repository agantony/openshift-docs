...
[id="about-release-467_{context}"]
== About release 4.6.7

*Release date*: 11 June 2025

This release of {product-title-short} 4.6 provides the following bug fixes:
...
//ROX-29716
* Before this update, image digest mismatches caused signature verification to fail, even when the signatures were valid. With this update, a check verifies image references against signatures, resolving the issue and allowing {ocp} to successfully verify image signatures for secure deployment.

[id="about-release-469_{context}"]
== About release 4.6.9

*Release date*: {ga-date-469}
...
[id="about-release-4610_{context}"]
== About release 4.6.10

*Release date*: {ga-date-4610}

This release addresses the following issues:

//ROX-30945
* Before this update, product container images were created with an older technology stack, resulting in inconsistent metadata such as container labels and Software Bill of Materials (SBOM) location. As a consequence, third-party integrations that relied on this metadata were potentially impacted.
+
With this release, container images are created with a new, updated technology stack. As a result, the metadata is now standardized, consistent and compliant with Red Hat best practices, ensuring stability for third-party integrations.
